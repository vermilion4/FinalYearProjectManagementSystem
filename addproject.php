...
/*else{
  //if user did not add a project with same name then check if a project is already added by someone with same title
  $sql = "select  * from student where fullname='".$_SESSION['fullname']."'";
  ...
  if ($result->num_rows>0){
    ...
    while($row = $result->fetch_assoc()){
        $foundPid = $row['pid'];
        $sql = "SELECT * FROM `tbl_projects` inner join student ON tbl_projects.studentid=student.studentId where pid=".$foundPid;
...
if($Flag==0){
    ...
    $sql1 = "DELETE  FROM images WHERE usprj='".$_SESSION['fullname'].'_'.$_POST['title']."'";
    ...
    if(array_filter($_FILES['files']['name'])){
        ...
        foreach($_FILES['files']['name'] as $key=>$val){
            ...
            $targetFilePath = $targetDir . $_SESSION['fullname'].'_'.$_POST['title'].'_'.$fileName;
            ...
            if(in_array($fileType, $allowTypes)){
                ...
                try {
                  ...
                  $insertValuesSQL .= "('".$_SESSION['fullname'].'_'.$_POST['title']."','".$fileName."'),";
        ...
        if(!empty($insertValuesSQL)){
            ...
            $insert = $conn->query("INSERT INTO images (usprj,src) VALUES $insertValuesSQL");
            if($insert){
              $targetFilePath ='';
                    ...
                    if(in_array($fileType, $allowTypes)){
                        ...
                        try {
                          move_uploaded_file($_FILES["files1"]["tmp_name"][0], $targetFilePath);
                            $prjFileFlag=1;
                            $targetFilePath =$_SESSION['fullname'].'_'.$_POST['title'].'_'.$fileName;
                        }
                       catch (\Throwable $th) {
                        //throw $th;
...
<!--  page banner -->
  <section id="page-banner" class="page-banner-main-block" style="background-image: url('images/bg/page-banner.jpg')">
    <div class="row">
  <div class="col-sm-2 text-center" style="float:left;"> <a href="index.php"><button type="submit" class="btn btn-default"> Back</button></a></div>
  
    <div class="col-sm-6"></div>
 <!-- <div class="col-sm-2" style="float:right;"> <a href="home.php"><button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-log-out"></span>&nbsp;Log Out</button></a></div> -->
<div class="col-sm-8">
    <center><p> Logged In:</p>
   <img src="<?php echo $_SESSION['img'];?>" style="width:120px; border-radius:50px;"/></center>
   <center> <h4 class=" text-center"><?php echo $_SESSION['fullname']; ?></h4></center>
...
<div class="col-sm-6">
 <div class="form-group">
  ...
  <label>Project Name: </label>
              ...
              <div class="form-group">
                <label> Area: </label>
                <input type="text" required class="form-control"  name="area" placeholder="Enter Area of domain" value="<?php if(isset($kfname)) echo $area;?>"/>
              </div>
              ...
              <div class="form-group"><label> Visibility: &nbsp;</label><br> 
